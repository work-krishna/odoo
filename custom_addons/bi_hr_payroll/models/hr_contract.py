...
class HrVersion(models.Model):
    ...
    def get_attribute(self, code, attribute):
        return self.env['hr.version.advantage.template'].search([('code', '=', code)], limit=1)[attribute]

    def set_attribute_value(self, code, active):
        for contract in self:
            if active:
                value = self.env['hr.version.advantage.template'].search([('code', '=', code)], limit=1).default_value
                contract[code] = value
            else:
                contract[code] = 0.0


class HrVersionAdvandageTemplate(models.Model):
    _name = 'hr.version.advantage.template'
