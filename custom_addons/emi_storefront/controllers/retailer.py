...
from odoo.addons.emi_storefront.controllers.common import (
    IMAGE_MIMETYPES, UploadError, current_vendor, read_upload, to_float, to_int,
)
...
class EmiRetailer(http.Controller):
    ...
    def retailer_listing(self, listing_id=None, **post):
        ...
        context = {
            'vendor': vendor, 'listing': listing, 'editable': editable, 'error': post.get('error'),
    ...
    def _save_listing(self, vendor, listing, post):
        ...
        vals = {
            ...
            'description_sale': (post.get('description') or '').strip(),
        }
    ...
    def retailer_listing_submit(self, listing_id, **post):
        ...
        try:
            ...
            return request.redirect(f'/my/retailer/listing/{listing.id}?' + url_encode({'error': str(error)}))
        return request.redirect('/my/retailer')
