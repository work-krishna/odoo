...
/**
 * Ask Fonepay directly for a QR code to pay this order, instead of the generic online-payment
 * link the customer would otherwise have to open (and then scan a second, real QR from) on their
 * own phone.
 *
 * @param {Object} order
 * @return {Promise<{reference: string, qrCode: string}|null>} `null` if this order isn't set up
 *         for Fonepay (or Fonepay rejected the request), so the caller falls back to the standard
 *         link-QR flow for whatever online payment provider is actually configured.
 */
async function requestFonepayQrCode(order) {
    try {
        const result = await rpc(`/pos/pay/fonepay/qr/${order.id}`, {
            access_token: order.access_token,
        });
        return result?.qr_code ? { reference: result.reference, qrCode: result.qr_code } : null;
    } catch (error) {
        ...
        if (onlinePaymentLines.length > 0) {
            ...
            for (const onlinePaymentLine of onlinePaymentLines) {
                ...
                if (!lastOrderServerOPData.isPaid) {
                    ...
                    // Try to get Fonepay's own QR code directly; fall back to the standard
                    // "open this link on your phone" QR for any other online payment provider,
                    // or if Fonepay isn't set up for this order.
                    const fonepayQr = await requestFonepayQrCode(this.order);
                    ...
                    // Fonepay has no webhook into Odoo: nothing else will check the payment
                    // status while this popup is open, so poll for it ourselves. Once the
                    // transaction is confirmed server-side, the existing
                    // `ONLINE_PAYMENTS_NOTIFICATION` bus flow (unmodified, see pos_store.js)
                    // resolves `onlinePaymentResolver` below and closes this popup, exactly like
                    // it does for any other online payment provider.
                    let fonepayPollTimer = null;
                    if (fonepayQr) {
                        fonepayPollTimer = setInterval(() => {
                            rpc("/payment/fonepay/poll", { reference: fonepayQr.reference }).catch(
                    ...
                    const qrCodePopupCloser = this.pos.dialog.add(
                        ...
                        {
                            onClose: () => {
                                clearInterval(fonepayPollTimer);
