...
class EmiScheduleLine(models.Model):
    ...
    state = fields.Selection(
        [('upcoming', 'Upcoming'), ('due', 'Due'), ('overdue', 'Overdue'), ('partial', 'Partially Paid'), ('paid', 'Paid')],
    ...
    @api.depends('due_date', 'amount', 'due_move_id', 'due_move_line_id.amount_residual', 'due_move_line_id.reconciled')
    ...
    def _cron_refresh_overdue(self):
        ...
        open_lines = self.sudo().search([('state', 'in', ('upcoming', 'due'))])
    ...
    def _post_installment_entry(self, date=None):
        """Post the finance company's installment entry for these lines."""
        for line in self.filtered(lambda l: not l.due_move_id).sorted('number'):
            ...
            move = self.env['account.move'].sudo().with_company(company).create({
                ...
                'ref': f"{app.name} installment {line.number}/{app.tenure_months}",
                'emi_application_id': app.id,
                'line_ids': [
                    (0, 0, {'name': f"{app.name} installment {line.number}", 'account_id': receivable.id,
